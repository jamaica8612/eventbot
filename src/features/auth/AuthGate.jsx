  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 11 }}>
      <span style={{
        ...
        background: 'linear-gradient(140deg, var(--accent), var(--home))',
        display: 'grid', placeItems: 'center',
        boxShadow: '0 6px 18px -6px var(--accent)',
      }}>
        <Icon name="trophy" size={size * 0.5} stroke={2} style={{ color: '#fff' }} />
      </span>
      <span style={{ fontSize: size * 0.46, fontWeight: 800, letterSpacing: '-.02em', color: 'var(--text)' }}>
        당첨노트
      </span>
    </span>
  );
...
export function AuthGate({ stage, theme, onToggleTheme, onLogin, onSwitchAccount, account }) {
  return (
    <div style={{
      ...
      {stage === 'login' && (
        <div style={{ width: '100%', maxWidth: 380, textAlign: 'center', animation: 'popIn .35s var(--ease-out)' }}>
          ...
          <div style={{
            ...
            borderRadius: 'var(--r-lg)', padding: 28, boxShadow: 'var(--shadow-2)',
          }}>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', transition: 'filter .13s var(--ease)',
              }}
            >
              <Icon name="google" size={20} /> Google 계정으로 로그인
            </button>
      ...
      {stage === 'pending' && (
        <div style={{ width: '100%', maxWidth: 400, textAlign: 'center', animation: 'popIn .35s var(--ease-out)' }}>
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px', fontSize: 12.5, color: 'var(--text-3)' }}>
              승인되면 바로 이용할 수 있습니다.
            </p>
            <Button variant="outline" icon="undo" full onClick={onSwitchAccount}>
              다른 계정으로 로그인
            </Button>
          </div>
        </div>
