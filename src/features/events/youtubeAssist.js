// YouTube 응모 도우미 — 실제 엔진 배선.
// 원본 components/EventBodyToggle.jsx 의 유튜브 수집/댓글 생성 로직을 이식.
...
// mode: 'context'(정보수집) | 'candidates'(댓글 생성)
export async function fetchYoutubeContextPayload({ youtubeLink, event, mode, signal }) {
...
function getYoutubeTranscriptLines(event, context) {
  ...
  if (crawledTranscript?.text) {
    return splitLines(crawledTranscript.text);
  }
  if (context?.transcript?.text) {
    return splitLines(context.transcript.text);
  }
  return [];
}
