...
import { hasRemoteDataConfig, loadRemoteEvents as loadStoredRemoteEvents } from '../storage/eventDataStorage.js';
...
async function loadRemoteEvents(): Promise<EventModel[]> {
  if (hasRemoteDataConfig) {
    const supabaseEvents = await loadStoredRemoteEvents();
...
export function useEvents(initialRemoteEvents: EventModel[] | null = null): {
  ...
  useEffect(() => {
    ...
    loadRemoteEvents().then((remoteEvents) => {
      ...
      const normalizedEvents = hasRemoteDataConfig
        ? nextEvents
