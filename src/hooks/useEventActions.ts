...
import {
  hasRemoteDataConfig,
  updateRemoteEventDetails,
  updateRemoteEventState,
...
function persistRemote(eventId: string, patch: EventPatch, setSyncNotice: SetSyncNotice): void {
  setSyncNotice(null);
  updateRemoteEventState(eventId, patch)
...
export function useEventActions({ events, setEvents, setSyncNotice }: UseEventActionsArgs) {
  useEffect(() => {
    ...
    async function flushQueue() {
      ...
      for (const item of queue) {
        if (!item) continue;
        try {
          await updateRemoteEventState(item.eventId, item.patch);
          successCount += 1;
        } catch (error) {
  ...
  const updateStatus = useCallback(
    (eventId: string, status: EventStatus) => {
      ...
      if (!hasRemoteDataConfig) {
        saveEventStatus(eventId, status);
      }
      if (!hasRemoteDataConfig && status === 'skipped') {
  ...
  const updateDeadline = useCallback(
    (eventId: string, meta: EventPatch) => {
      ...
      setSyncNotice(null);
      updateRemoteEventDetails(eventId, patch)
