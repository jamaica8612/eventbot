    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 90, background: 'rgba(15,18,30,.45)', backdropFilter: 'blur(2px)', display: 'flex', justifyContent: 'flex-end', animation: 'fadeIn .18s' }}>
      <div onClick={(e) => e.stopPropagation()} className="filter-drawer" style={{
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          <Section title="일반">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {onLock && <Btn variant="outline" icon="lock" onClick={onLock} style={{ flex: 1 }}>잠금</Btn>}
              </div>
            </div>
          </Section>
          ...
          {(showGeminiApiKey || showCommentPrompt) && <Section title="댓글 생성 설정" sub="YouTube 응모 댓글 자동 생성에 사용돼요.">
            {showGeminiApiKey && (
              <label style={{ ...lbl, display: 'block', marginBottom: showCommentPrompt ? 12 : 0 }}>내 Gemini API 키 <span style={{ fontWeight: 400, color: 'var(--text-3)' }}>(선택)</span>
