...
import {
  hasRemoteDataConfig,
  loadEventBootstrap,
  loadRemoteCommentSettings,
  loadRemoteCrawlerStatus,
  loadRemoteFilterSettings,
  saveRemoteCommentSettings,
  saveRemoteFilterSettings,
  triggerRemoteCrawler,
} from '../storage/eventDataStorage.js';
import {
...
import { Avatar, Badge, Brandmark, Btn, Empty, IconBtn, Spinner } from './components/primitives.jsx';
...
const SAVED_TAB_KEY = 'eventbotActiveTab';
const USER_TAB_IDS = new Set([...NAV.map((item) => item.id), 'excluded']);
...
function AppV2Main({ theme, toggleTheme, profile, bootstrap, onLock }) {
  ...
  useEffect(() => {
    ...
    if (!hasRemoteDataConfig) { didLoadFilter.current = true; return; }
    loadRemoteFilterSettings()
  ...
  useEffect(() => {
    if (bootstrap) return;
    if (!hasRemoteDataConfig) return;
    loadRemoteCommentSettings().then((r) => { if (r) setCommentSettings(normalizeCommentSettings(r)); }).catch(() => {});
  }, []);
  useEffect(() => {
    if (bootstrap) return;
    if (!hasRemoteDataConfig) return;
    let m = true;
    loadRemoteCrawlerStatus().then((s) => { if (m) setCrawlerStatus(s); }).catch(() => { if (m) setCrawlerStatus(null); });
    return () => { m = false; };
  }, []);
  useEffect(() => {
    ...
    if (hasRemoteDataConfig) { saveRemoteFilterSettings(filterSettings).catch(() => {}); return; }
  ...
  async function handleManualCrawl() {
    ...
    try {
      if (!hasRemoteDataConfig) throw new Error('Supabase 설정이 필요합니다.');
      const payload = await triggerRemoteCrawler();
      if (payload?.crawlStatus) setCrawlerStatus(payload.crawlStatus);
      setSyncNotice({ type: 'success', message: '크롤링 작업을 요청했습니다. 완료까지 몇 분 걸릴 수 있습니다.' });
  ...
  async function handleSaveComment() {
    ...
    try {
      if (hasRemoteDataConfig) await saveRemoteCommentSettings(normalized);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(SAVED_TAB_KEY, tab);
    const url = new URL(window.location.href);
    if (tab === 'waiting') url.searchParams.delete('tab');
    ...
    <div className="app">
      ...
      <aside className="sidebar">
        ...
        <div style={{ padding: 12, borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
          ...
          {onLock && <IconBtn name="lock" title="잠금" onClick={onLock} style={{ flex: 1 }} />}
        </div>
      </aside>
  ...
  if (compact) {
    return (
      <button
        ...
        style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '4px 2px', background: 'none', border: 'none', position: 'relative', cursor: 'pointer' }}
      >
...
function loadInitialTab() {
  ...
  const savedTab = window.localStorage.getItem(SAVED_TAB_KEY);
  const requested = queryTab || savedTab || 'waiting';
  if (requested === 'admin') return requested;
  if (!USER_TAB_IDS.has(requested)) return 'waiting';
  return requested;
}
