...
- **1차 수집 대상(확정)**:
  - 뽐뿌 국내: `ppomppu.co.kr/zboard/zboard.php?id=ppomppu`
  - 뽐뿌 해외: `id=ppomppu4`
  - 뽐뿌 알리: (사이트에서 정확한 게시판 id 확인)
  - 에펨코리아 핫딜: `fmkorea.com/hotdeal`
  - 클리앙 알뜰구매: `clien.net/service/board/jirum`
- 사이트별 파서: `ppomppu.py`(국내/해외/알리 공용, 게시판 id만 다름), `fmkorea.py`, `clien.py` — 핫딜 목록 파싱(제목/가격/쇼핑몰/카테고리/추천/댓글/링크/썸네일/게시시각/품절).
- 확장 후보: 루리웹·퀘이사존(후속).

> 수집은 여러 커뮤니티에서 하지만 결과는 **하나의 앱(당첨노트)** 핫딜 탭에 통합된다. 별도 사이트가 아니다.
