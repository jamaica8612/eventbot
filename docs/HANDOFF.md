...
이벤트 딸각 웹앱은 온라인 이벤트를 자동으로 수집하고, 편하게 관리하며, 빠르게 응모할 수 있게 도와주는 웹앱입니다.

목표 흐름은 `크롤러 -> DB -> 웹앱 -> 딸각 관리 -> 응모`입니다.

## 앱 방향

- 모바일과 PC 모두에서 같은 데이터와 상태를 공유하는 하나의 반응형 웹앱입니다.
- 구조가 단순해야 합니다.
- 디자인 품질도 중요하게 봅니다.
- 무료 또는 저비용 운영을 선호합니다.
...
- `필터설정` 패널에서 제외 키워드, 숨길 플랫폼을 조절할 수 있습니다.
...
- `public/sw.js` 서비스 워커를 추가하고, 앱 설치 버튼을 붙였습니다.
...
- 당첨 판별, 오늘마감, 검색, 응모함 정렬은 규칙 기반으로 유지합니다.
- 모바일 앱과 PC 앱을 따로 만들지 않습니다.
- 웹앱과 크롤러는 강하게 얽히지 않게 분리합니다.
...
- 배포 준비는 GitHub Pages 기준으로 `docs/DEPLOYMENT.md`에 정리했습니다.
